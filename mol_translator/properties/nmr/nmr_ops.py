...
def get_coupling_types(aemol):
    ...
    for t, type in enumerate(aemol.structure['types']):
        ...
        for t2, type2 in enumerate(aemol.structure['types']):
            ...
            if type > type2:
                targetflag = str(
                    aemol.structure['path_len'][t][t2]) + 'J' + p_table[type] + p_table[type2]
            else:
                targetflag = str(
                    aemol.structure['path_len'][t][t2]) + 'J' + p_table[type2] + p_table[type]
...
def scale_chemical_shifts(aemol, scaling={1: [-1.0594, 32.2293], 6: [-1.0207, 187.4436], 7: [-1.0139, -148.67], 19: [-1.0940, 173.02], 31: [-1.2777, 307.74]}):
    ...
    for t, type in enumerate(aemol.structure['types']):
        ...
        aemol.atom_properties['shift'][t] = (
            aemol.atom_properties['shift'][t] - scaling[type][1]) / scaling[type][0]
